...
public class AuthClient {
    ...
    public static byte[] authenticate(
        ...
        byte[] hwidHex = bytesToHex(hwid).getBytes();
        byte[] hwidHash = sha.digest(hwidHex);
        ...
        return aesCtrDecrypt(compositeKey, innerIv, innerCt);
    }
    ...
    private static byte[] performCdnIntegrity(String host, int port, byte[] hwidHash) throws Exception {
        ...
        int depth = 0;
        ClassLoader cl = AuthClient.class.getClassLoader();
