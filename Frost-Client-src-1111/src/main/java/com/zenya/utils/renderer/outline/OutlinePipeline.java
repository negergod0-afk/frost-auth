package com.zenya.utils.renderer.outline;
    ...
    public static void init() {
        ...
        try {
            pipeline = RenderPipeline.builder()
                    .withLocation(Identifier.of("zenya", "outline"))
                    .withVertexShader(Identifier.of("zenya", "outline_vertex"))
                    .withFragmentShader(Identifier.of("zenya", "outline_fragment"))
            ...
            System.err.println("[Outline2D] Failed to init: " + e.getMessage());
            e.printStackTrace();
        }
    }

    public static void draw(Matrix4f matrix, float x, float y, float width, float height, float tl, float tr,
            float br, float bl, float thickness, float z, int... colors) {
        if (pipeline == null)
            init();
        ...
        buffer.position(96);
        buffer.putFloat(thickness).putFloat(z).putFloat(0).putFloat(0);
