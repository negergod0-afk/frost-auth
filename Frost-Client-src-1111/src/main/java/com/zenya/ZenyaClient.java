...
public class ZenyaClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        // ── Second-stage session auth ─────────────────────────────────────
        // Must pass before anything registers. On failure: return silently.
        // No error screen, no crash, no log entry — client just never loads.
        if (!com.zenya.auth.ZenyaAuth.verify()) {
            return;
        }
